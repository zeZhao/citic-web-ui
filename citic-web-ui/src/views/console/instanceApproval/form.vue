...
<template>
    <div>
            ...
            <div>
                <div class="status-box">
                    <div class="finish clearfix" v-if="statusCode === 1">
                        <Icon type="android-checkmark-circle" size="70" class="finish-icon" />
                    ...
                    <div class="finish clearfix" v-else-if="statusCode === 5">
                        <Icon type="close-circled" size="70" class="finish-icon" />
                        <div class="finish-txt">
        ...
        <!--查询操作日志-->
        <Modal v-model="approvalDialog" class="instance-approval-dialog">
...
export default {
    data() {
        return {
            ...
            action:
                ...
                this.$API_ENUM.COMMON_UPLOAD_FILE.Format("InstanceApprove"),
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (h, params) => {
                        return [
                            h("p", (params.row.message || "").HTMLDecode())
                        ];
                    }
                },
                {
    ...
    created() {
        const { statusCode, id, tenantName, orderId } = this.$route.query;
    ...
    methods: {
        getData() {
            this.$get(this.$API_ENUM.GENERAL_INSTANCE_APPROVE_DETAIL, {
                id: this.id,
         ...
         */
        getApproveLog() {
            this.$get(this.$API_ENUM.GENERAL_INSTANCE_APPROVE_LOG, {
                instanceId: this.id
            }).then(res => {
                if (resOk(res)) {
        ...
        async approvalSubmit(submit) {
            if (submit) {
                const res = await this.$post(
                    this.$API_ENUM.GENERAL_INSTANCE_APPROVE,
                    {
